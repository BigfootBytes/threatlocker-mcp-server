        ...
        messages: [
          {
            ...
            content: {
              ...
              text: [
                ...
                ``,
                `**Steps:**`,
                `1. Use \`action_log\` with action "search", actionId 99 (Any Deny), and the timeframe above to find recent denials${hostname !== '*' ? ` for hostname "${hostname}"` : ''}${path ? ` matching path "${path}"` : ''}.`,
                `2. Review the denial entries — note the full path, process path, hash, and certificate information.`,
                `3. Use \`computers\` to look up the affected computer and check its current mode (Secure, Learning, Installation).`,
                `4. Use \`applications\` with action "match" to check if the denied file matches any known application.`,
                `5. Use \`policies\` to review existing policies for the computer's group — determine if a policy should be created or modified.`,
      ...
      messages: [
        {
          ...
          content: {
            ...
            text: [
              ...
              ``,
              `**Steps:**`,
              `1. Use \`approval_requests\` with action "count" to get the total number of pending requests.`,
              `2. Use \`approval_requests\` with action "list", statusId 1 (Pending), ordered by "datetime" descending to see the most recent requests.`,
              ...
              `5. For high-priority or recurring requests, use \`applications\` with action "research" to check the ThreatLocker research rating and risk assessment.`,
        ...
        messages: [
          {
            ...
            content: {
              ...
              text: [
                ...
                ``,
                `**Steps:**`,
                `1. Use \`computers\` with kindOfAction "Computer Mode" to get an overview of computers in each mode (Secure, Learning, Installation, MonitorOnly). Flag any computers not in Secure mode.`,
                `2. Use \`computers\` with kindOfAction "NeedsReview" to identify computers that need attention.`,
                `3. Use \`action_log\` with actionId 99 (Any Deny) for the ${timeframe} to get recent denial activity. Use groupBys [8, 17] (Application Name, Asset Name) for a summary view.`,
                `4. Use \`approval_requests\` with action "count" to check pending approval requests.`,
                `5. Use \`system_audit\` with action "search" for the ${timeframe} to review portal activity (logins, policy changes, configuration modifications).`,
        ...
        messages: [
          {
            ...
            content: {
              ...
              text: [
                ...
                ``,
                `**Steps:**`,
                `1. Use \`computers\` with action "list" and searchText "${computerName}" to find the computer. Note its ID, group, current mode, and last check-in time.`,
                `2. Use \`maintenance_mode\` with action "get_history" and the computer ID to review recent maintenance mode activity (installation mode, learning mode, etc.).`,
                `3. Use \`computer_groups\` to get the computer's group details and understand which policies apply.`,
                `4. Use \`action_log\` with action "search", the computer's hostname, and actionId 99 (Any Deny) for the last 7 days to review recent denials on this machine.`,
                `5. Use \`action_log\` with action "search", the computer's hostname, and actionId 1 (Permit) with groupBys [8] (Application Name) to see what applications are actively running.`,
